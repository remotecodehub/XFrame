...
export function setTool(nextTool) {
    ...
    if (tool !== 'Translate' && tool !== 'Rotate') cancelDrag();
}
...
function pointerDown(event) {
    if ((tool !== 'Translate' && tool !== 'Rotate') || event.button !== 0 || drag) return;
    ...
    const startWorld = tool === 'Rotate' ? renderer?.getInteractionPoint?.(point.x, point.y, position, axisVector(axis)) : null;
    drag = {
        ...
        scale,
        startWorld,
        last: cloneTransform(selected.Transform)
    };
...
function pointerMove(event) {
    ...
    if (tool === 'Translate') {
        ...
        position[axisIndex(drag.axis)] += delta * TRANSLATE_SENSITIVITY;
    } else if (tool === 'Rotate') {
        ...
        while (delta < -180) delta += 360;
        rotation[axisIndex(drag.axis)] += delta * ROTATE_SENSITIVITY;
    }
...
function finishDrag(event) {
    const current = drag;
    applyPreview();
    const t = cloneTransform(current.last);
    const p = vectorValue(t.Position), r = vectorValue(t.Rotation), s = vectorValue(t.Scale, [1, 1, 1]);
    ...
    // Keep the committed transform in this JS scene until .NET sends the same authoritative state.
    // This prevents pointerup from visually reverting the object while the async callback is processed.
    drag = null;
    event.preventDefault();
    event.stopImmediatePropagation();
}
...
function cancelDrag() {
    if (!drag) return;
    try { if (canvas?.hasPointerCapture?.(drag.pointerId)) canvas.releasePointerCapture(drag.pointerId); } catch { }
    drag = null;
}
...
function axisVector(a){return a==='X'?[1,0,0]:a==='Y'?[0,1,0]:[0,0,1];}
