...
export async function initialize(canvasId, callback) {
    ...
    renderer = await import('./xframe-webgpu.js');
...
export function setScene(json) {
    ...
    if (!incoming) return;
    if (drag) {
        const current = scene.Objects?.find(o => o.Id === drag.objectId);
        const incomingObject = incoming.Objects?.find(o => o.Id === drag.objectId);
        if (current && incomingObject) current.Transform = cloneTransform(drag.last);
    }
    scene = incoming;
    if (drag) {
...
function pointerDown(event) {
    ...
    const selected = scene.Objects?.find(o => o.Id === scene.SelectedObjectId);
    if (!selected) return;
    const point = pointerPoint(event);
    const labels = axisLabels();
    const axis = hitAxis(point.x, point.y, labels);
    if (axis === 'None') return;
    ...
    const center = gizmoCenter(labels);
    const direction = normalize2([labels[axis].x - center.x, labels[axis].y - center.y]);
    drag = { pointerId: event.pointerId, objectId: selected.Id, axis, start: point, direction, position, rotation, scale, last: cloneTransform(selected.Transform) };
...
function pointerMove(event) {
    ...
    const point = pointerPoint(event);
    const delta = (point.x - drag.start.x) * drag.direction.x + (point.y - drag.start.y) * drag.direction.y;
    const position = [...drag.position];
    position[axisIndex(drag.axis)] += delta * WORLD_PER_PIXEL;
    drag.last = { Position:{X:position[0],Y:position[1],Z:position[2]}, Rotation:{X:drag.rotation[0],Y:drag.rotation[1],Z:drag.rotation[2]}, Scale:{X:drag.scale[0],Y:drag.scale[1],Z:drag.scale[2]} };
    applyPreview();
    event.preventDefault();
    event.stopImmediatePropagation();
}
...
function finishDrag(event) { const current=drag; applyPreview(); drag=null; try{if(canvas?.hasPointerCapture?.(current.pointerId))canvas.releasePointerCapture(current.pointerId);}catch{} const t=current.last,p=vectorValue(t.Position),r=vectorValue(t.Rotation),s=vectorValue(t.Scale,[1,1,1]); dotnet?.invokeMethodAsync('OnTransformCommitted',current.objectId,p[0],p[1],p[2],r[0],r[1],r[2],s[0],s[1],s[2]); event.preventDefault(); event.stopImmediatePropagation(); }
...
function axisLabels(){const result={};const root=canvas?.parentElement;const canvasRect=canvas?.getBoundingClientRect();if(!root||!canvasRect)return result;for(const element of root.querySelectorAll('span')){const axis=element.textContent?.trim();if(!AXES.includes(axis))continue;const rect=element.getBoundingClientRect();result[axis]={x:rect.left+rect.width/2-canvasRect.left,y:rect.top+rect.height/2-canvasRect.top};}return result;}
